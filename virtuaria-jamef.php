<?php
/**
 * Plugin Name: Virtuaria Jamef para Woocommerce
...
if ( ! class_exists( 'Virtuaria_Jamef' ) ) {
	/**
	 * Class definition
	 */
	class Virtuaria_Jamef {
		/**
		...
		public function add_jamef_shipping_method( $methods ) {
			$methods['virtuaria_jamef'] = 'WC_Jamef_Shipping';
			return $methods;
		}
		...
		public function shipping_delivery_forecast( $shipping_method ) {
			...
			if ( isset( $meta_data['_estimate_delivery'] ) ) {
				echo '<p><small>' . esc_html( sprintf( __( 'Previsão de entrega: %s', 'virtuaria-jamef' ), $meta_data['_estimate_delivery'] ) ) . '</small></p>';
			}
		}
	}

	add_action( 'plugins_loaded', array( 'Virtuaria_Jamef', 'get_instance' ) );
}
